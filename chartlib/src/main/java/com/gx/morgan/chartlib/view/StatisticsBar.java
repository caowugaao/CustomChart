...
public class StatisticsBar extends CustomView {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        for (int i = 0; i <contentDataSize ; i++) {
            contentDatas.add(new ContentData(i+1,datas[i]));
        }
    }
...
//    }
    ...
    private void drawStatisticBar(Canvas canvas) {
        ...
        int yCoodinateMaxData= yCoodinateDatas.get(yDataSize-1)+yCoodinateDatas.get(yDataSize-1)-yCoodinateDatas.get(yDataSize-2);//y轴最大数字
        ...
        for (int i = 0,size=contentDatas.size(); i <size ; i++) {
            ...
            int left= (int) (coodinateXStartX+(contentData.x*1.0/xCoodinateMaxData)*xCoordinateDistance-xSpacing/2-statisticBarWidth/2);
